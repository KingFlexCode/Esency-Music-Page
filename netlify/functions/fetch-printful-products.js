...
export async function handler() {
  ...
  const STORE_ID = "17034634"; // Update this if needed
  ...
  try {
    ...
    // Map Printful data to front-end format
    const products = data.result.map((p) => ({
      id: p.id,
      name: p.name,
      thumbnail_url: p.thumbnail_url,
      price: 35.0, // Static placeholder; update from variants endpoint if needed
